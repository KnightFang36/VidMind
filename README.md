...
Build (or rebuild) the hybrid index for a video.

```jsonc
// request
{ "video_id": "abc123", "force": false }

// response
{ "video_id": "abc123", "chunks_indexed": 142 }
```
...
// request
{
  ...
  "history": [
    ...
    { "role": "assistant", "content": "It covers product strategy." }
  ]
}
...
// response
{
  "video_id": "abc123",
  "query": "What did they say about pricing?",
  "answer": "They mentioned tiered pricing starting at $9/mo...",
  "standalone_question": "What did the speakers say about pricing tiers?",
  "grounded": true,
  "sources": [
    {
      "video_id": "abc123",
      ...
      "start_seconds": 125.5,
      "timestamp": "2:05",
      "url": "https://www.youtube.com/watch?v=abc123&t=125s",
      "snippet": "Our pricing starts at nine dollars a month..."
    }
  ]
}
```

- `history` is optional; sending prior turns enables conversation memory.
- `grounded` is `false` when the hallucination guard declines to answer.
...
├── backend/
│   ├── app/
│   │   ├── api/v1/routes/    # chat.py, index.py — endpoints
│   │   ├── core/            # configuration / settings
│   │   ├── schemas/         # request & response models
│   │   ├── services/        # ingestion, embedding, indexing, retrieval,
│   │   │                    #   memory, guardrails, cache, rag orchestration
│   │   └── main.py          # FastAPI entry point
│   ├── data/faiss_indexes/  # persisted per-video FAISS + BM25 + parent stores
│   ├── data/embedding_cache/# cached embedding vectors
│   ├── tests/
...
│   │   ├── content/         # YouTube page detection
│   │   ├── sidebar/         # App, components (incl. timestamped source cards)
│   │   ├── popup/
│   │   ├── api/             # backend client (chat + index)
│   │   └── utils/
│   ├── package.json
│   └── vite.config.ts
└── README.md
