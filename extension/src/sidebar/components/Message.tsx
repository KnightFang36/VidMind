...
export function Message({ message, onSourceClick }: MessageProps) {
  if (message.role === "user") {
    return (
      <article className="flex flex-col items-end gap-1" aria-label="You">
        <span className="text-[10px] font-semibold tracking-[0.08em] text-[#7B8392] uppercase">
          You
        </span>
  ...
  return (
    <article className="flex items-start gap-2.5" aria-label="VidMind">
      ...
      <div className="min-w-0 flex-1 pt-0.5">
        <p className="mb-1 text-[10px] font-semibold tracking-[0.08em] text-[#7B8392] uppercase">
          VidMind
        </p>
